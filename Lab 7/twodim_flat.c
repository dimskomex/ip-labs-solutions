...
// Συνάρτηση για την εκτύπωση του πίνακα με αντίστροφη σειρά στοιχείων σε κάθε
// γραμμή
...
// Συνάρτηση για την εκτύπωση όλων των στοιχείων σε μία σειρά
void print_flat(int rows, int cols, int array[rows][cols]) {
  for (int i = 0; i < rows; i++) {
    for (int j = 0; j < cols; j++) {
      printf("%d ", array[i][j]);
    }
...
int main() {
  ...
  // Εκτύπωση του πίνακα με αντίστροφη σειρά στοιχείων στις γραμμές
  printf("\nReversed array:\n");
  print_reverse(rows, cols, array);

  // Εκτύπωση όλων των στοιχείων σε μία σειρά
  printf("\nFlat array:\n");
  print_flat(rows, cols, array);

  return 0;
}

// Μεταγλώττιση:
// gcc twodim_flat.c -o twodim_flat
// Εκτέλεση:
// ./twodim_flat
// Έξοδος:
// Original array:
...
// Flat array:
// 25 33 39 43 45 20 28 34 38 40 15 23 29 33 35